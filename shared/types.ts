...
export type Location = { kind: 'ground'; x: number; y: number } | { kind: 'held'; actor: string } | { kind: 'removed' };
export interface Entity {
  id: string; name: string; kind: 'actor' | 'item' | 'fixture'; location: Location;
  description: string; icon: string;
  props: { portable?: boolean; fragile?: boolean; solid?: boolean; size?: number; gap?: number; locked?: boolean; open?: boolean; key?: string; heal?: number; price?: number; owner?: string; noise?: number; color?: string };
}
...
export type TransformOp = { kind: 'transform'; entity: string; rule: 'heal' | 'rest' | 'wake' | 'open' | 'close' | 'permit' | 'settle' | 'look'; target?: string; x?: number; y?: number };
...
export interface World {
  ...
  phase: ReactionPhase | null; receipts: Record<string, { version: number; ok: boolean; reason?: string }>;
}
...
export interface PublicActor { id: string; hp: number; maxHp: number; fatigue: number; wakefulness: 'awake' | 'asleep'; attention: Actor['attention']; facing: Actor['facing']; mood: string; }
export interface PublicState {
  ...
  objective: World['objective']; events: WorldEvent[]; issues: Issue[]; busy: boolean;
}
...
export type DirectIntent = { kind: 'move'; x: number; y: number } | { kind: 'interact' | 'drop'; entity: string } | { kind: 'wait' | 'rest' };
