...
export interface CombatFact {
  eventId: string; incidentId: string; actorId: string; victimId: string;
  targetWasEnemy: boolean; targetWasHostile: boolean; unprovoked: boolean;
  source: 'weapon' | 'throw' | 'awakening'; outcome: 'hit' | 'defeat' | 'death';
}
export interface BehaviorTrait { id: string; label: string; description: string; victimIds: string[]; eventIds: string[]; }
export interface KnownBehavior {
  facts: (CombatFact & { knowledge: 'witnessed' | 'reported'; sourceEventIds: string[]; reporterId?: string })[];
  traits: (BehaviorTrait & { knowledge: 'witnessed' | 'reported' })[];
}
...
export interface ActorView { actor: Actor; self: Entity; entities: Entity[]; observations: Observation[]; width: number; height: number; walls: Point[]; tick: number; version: number; knownIssues: Issue[]; knownBehavior?: KnownBehavior; }
