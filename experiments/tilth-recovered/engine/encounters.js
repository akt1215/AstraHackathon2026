...
export class Encounters{
 constructor({actors=ACTORS,mover=advance,onProvoked=()=>{},onFeedback=()=>{},onDefeated=()=>{},onCombat=()=>{}}={}){this.actors=actors.map(a=>({...a,maxHp:a.hp,home:{x:a.x,y:a.y},direction:'down',hostile:false,attack:null,ready:0,downUntil:0,peaceAt:0,nextRoam:0,walk:false,bubble:'',bubbleUntil:0,revision:0}));this.mover=mover;this.onProvoked=onProvoked;this.onFeedback=onFeedback;this.onDefeated=onDefeated;this.onCombat=onCombat;this.time=0;this.hp=280;this.maxHp=280;this.swing=null;this.invulnerable=0;this.guardUntil=0;this.potionReady=0;this.floats=[]}
 ...
 hit(a,damage,context={}){
  if(a.attackable===false||a.hp<=0||a.downUntil||!Number.isFinite(damage)||damage<=0)return null;
  const targetWasEnemy=Boolean(a.enemy),targetWasHostile=Boolean(a.hostile),first=!a.hostile,amount=Math.min(a.hp,damage);
  if(!a.combatIncident||!a.hostile)a.combatIncident=context.incidentId??globalThis.crypto.randomUUID();
  a.hp=Math.max(0,a.hp-amount);a.hostile=true;a.worldSleeping=false;a.peaceAt=this.time+14000;
  this.floats.push({x:a.x,y:a.y,text:`−${amount}`,until:this.time+900});if(first)a.revision++;
  if(a.hp===0){a.downUntil=this.time+18000;a.hostile=false;a.attack=null;a.revision++;}
  const fact={eventId:context.eventId??globalThis.crypto.randomUUID(),incidentId:a.combatIncident,actorId:context.actorId??'player',victimId:a.id,targetWasEnemy,targetWasHostile,unprovoked:!targetWasEnemy&&!targetWasHostile,source:context.source??'weapon',outcome:a.hp===0?'defeat':'hit',hp:a.hp,damage:amount,position:{x:a.x,y:a.y,room:a.sceneId??null},attackerPosition:context.attackerPosition?{x:context.attackerPosition.x,y:context.attackerPosition.y,room:a.sceneId??null}:undefined};
  this.onCombat(fact);
  if(this.time-(a.lastReaction??-Infinity)>30000){a.lastReaction=this.time;this.say(a,a.warning);this.onProvoked(a)}
  if(a.hp===0){this.say(a,a.enemy?'Defeated':'Enough… I yield.');this.onFeedback(`${a.name} ${a.enemy?'defeated':'yielded'}.`);this.onDefeated(a);}
  return fact;
 }
 ...
 if(this.swing&&!this.swing.done&&now>=this.swing.impact){this.swing.done=true;const s=this.swing;for(const a of this.actors)if(a.attackable!==false&&!a.downUntil&&inStrike(player,a,s.direction,attackStats(s.weapon,s.kind),this.mover))this.hit(a,attackStats(s.weapon,s.kind).damage,{source:'weapon',attackerPosition:player})}
 if(this.generated){const g=this.generated,times=impactTimes(g.move);while(g.next<times.length&&now>=g.start+g.duration*times[g.next]){g.next++;for(const a of this.actors)if(a.attackable!==false&&!a.downUntil&&inStrike(player,a,g.direction,moveStats(g.move),this.mover))this.hit(a,g.move.damage,{source:'awakening',attackerPosition:player})}if(g.next===times.length)this.generated=null;}
 for(const a of this.actors){a.walk=false;if(a.downUntil){if(a.persistentDefeat)continue;if(now>=a.downUntil){a.downUntil=0;a.hp=a.maxHp;a.x=a.home.x;a.y=a.home.y;a.ready=now+2000;a.combatIncident=null;a.revision++}continue}
 ...
 if(a.hostile&&(distance>210||now>a.peaceAt)){a.hostile=false;a.combatIncident=null;a.attack=null;a.revision++;a.ready=now+3500;this.say(a,a.enemy?'Run, then.':'Stay back. I am done fighting.');}
