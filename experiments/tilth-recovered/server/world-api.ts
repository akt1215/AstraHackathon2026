...
const knowledge=z.enum(['witnessed','reported']);
const combatFact=z.object({eventId:id,incidentId:id,actorId:id,victimId:id,targetWasEnemy:z.boolean(),targetWasHostile:z.boolean(),unprovoked:z.boolean(),source:z.enum(['weapon','throw','awakening']),outcome:z.enum(['hit','defeat','death']),knowledge,sourceEventIds:z.array(id).min(1).max(120),reporterId:id.optional()}).refine(fact=>fact.actorId!==fact.victimId&&(fact.knowledge!=='reported'||Boolean(fact.reporterId)),'Combat knowledge requires a distinct victim and a source for reports.');
const behaviorTrait=z.object({id,label:text,description:text,victimIds:z.array(id).max(120),eventIds:z.array(id).max(120),knowledge});
const knownBehavior=z.object({facts:z.array(combatFact).max(120),traits:z.array(behaviorTrait).max(8)});
...
const viewSchema=z.object({actor,self:entity,entities:z.array(entity).max(100),observations:z.array(observation).max(120),width:number.int().positive().max(1000),height:number.int().positive().max(1000),walls:z.array(point).max(1500),tick:number.int().min(0),version:number.int().min(0),knownIssues:z.array(issue).max(100),knownBehavior:knownBehavior.optional()}).refine(view=>view.actor.id===view.self.id&&view.self.kind==='actor','Actor and self must match.');
