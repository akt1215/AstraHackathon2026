...
export function parseDecision(raw: unknown, view: ActorView): Decision {
  ...
  const evidence = new Set([
    ...
    ...(view.knownBehavior?.facts ?? []).flatMap((fact) => [fact.eventId, ...fact.sourceEventIds]),
  ]);
...
Combat history: view.knownBehavior contains only this actor's own actions or combat facts this actor witnessed or received in a sourced report, plus traits derived from those facts. There is no global player reputation available to NPCs. Keep reported allegations distinct from firsthand knowledge; never infer unseen incidents or share a trait's unseen evidence. A defeat is not a death: characters may yield and recover. Only outcome=death confirms death. Aggressor and Ruthless describe repeated unprovoked attacks on distinct peaceful characters; fighting hostile or enemy opponents is separate Battle-hardened experience. Killer and Serial killer require confirmed deaths of peaceful victims after unprovoked attacks, never defeat counts. Use the exact provided descriptions and evidence; labels summarize recorded behavior, not an immutable personality, permission to invent abilities, or a restriction on the player's vocabulary. You may discuss or react to your known history and relationships, but speech never commits an attack, kill, penalty or support decision. Cite only known fact eventIds or sourceEventIds when referring to combat evidence.
Primitive contract:
...
export function buildPrompt(view: ActorView, request: RequestKind): string {
  ...
  const { id, role, hp, maxHp, fatigue, wakefulness, awakenedTick, attention, facing, mood, goal, memories, evidence, tendencies, capabilities, relationships, coins, following, permission } = view.actor;
  const actorView: ActorView = {
    actor: { id, role, hp, maxHp, fatigue, wakefulness, awakenedTick, attention, facing, mood, goal, memories, evidence, tendencies, capabilities, relationships, coins, following, permission }, self: view.self, entities: view.entities, observations: view.observations,
    width: view.width, height: view.height, walls: view.walls, tick: view.tick, version: view.version, knownIssues: view.knownIssues,
    knownBehavior: view.knownBehavior,
  };
