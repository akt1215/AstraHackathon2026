import {formMove} from './engine/form-renderer.js';
...
const actionDefinition=name=>formMove(runtime.state.profile.form,name)||weaponMove(runtime.state.profile.weapon||'sword',name,ACTIONS[name]);
...
function move(dir){
 if(modal.open||creator?.dialog.open||performance.now()-attackStart<activeDuration())return;
...
function animateScene(now){
 ...
 if(attackKind==='Dodge'&&now-attackStart<activeDuration()+100){
   ...
   if(!document.hidden&&!modal.open&&!creator?.dialog.open&&dt){const [dx,dy]={up:[0,-1],down:[0,1],left:[-1,0],right:[1,0]}[facing];player=advance(player,dx*dt*.2,dy*dt*.2,currentRoom);}
 }
...
function requestQuest(){if(modal.open||creator?.dialog.open)return;if(currentRoom||Math.hypot(player.x-268,player.y-229)>58){toast('Speak to Rowan near the inn. Walk closer, then press F.');return}runtime.requestQuest();engineUI.show('quests')}
function useAwakening(){if(modal.open||creator?.dialog.open)return;const content=runtime.activeOffer()?.content,now=performance.now();if(!content){toast('Accept an awakening to unlock its skill.');return}if(now<generatedSkillReady)return;generatedSkillStart=now;generatedSkillReady=now+content.skill.cooldownMs;playSound('Rally');draw()}
...
function refreshCharacter(){
 ...
 for(const button of document.querySelectorAll('[data-skill]')){const slot=button.dataset.skill,move=formMove(profile.form,slot)||WEAPONS[profile.weapon||'sword'].moves[slot];if(!move)continue;const key={Slash:'Space / 1',Heavy:'Q',Spin:'E',Bash:'R'}[slot];button.title=`${key} · ${move.name}`;button.setAttribute('aria-label',`${move.name} (${key})`);if(slot==='Slash')button.querySelector('span').textContent=profile.form?'◈':icons[profile.weapon||'sword']}
 portrait();draw();
}
creator=installOnboarding({runtime,onSave:()=>{refreshCharacter();toast('Your adventure begins. Try your moves with Space, Q, E, and R.')}});
if(runtime.state.profile.onboarded)refreshCharacter();else creator.open();
