...
export class Encounters{
 ...
 hit(a,damage){const first=!a.hostile;a.hp=Math.max(0,a.hp-damage);a.hostile=true;a.peaceAt=this.time+14000;this.floats.push({x:a.x,y:a.y,text:`−${damage}`,until:this.time+900});if(first)a.revision++;if(this.time-(a.lastReaction??-Infinity)>30000){a.lastReaction=this.time;this.say(a,a.warning);this.onProvoked(a)}if(a.hp===0){a.downUntil=this.time+18000;a.hostile=false;a.attack=null;a.revision++;this.say(a,a.enemy?'Defeated':'Enough… I yield.');this.onFeedback(`${a.name} ${a.enemy?'defeated':'yielded'}.`);}}
 ...
 if(this.swing&&!this.swing.done&&now>=this.swing.impact){this.swing.done=true;const s=this.swing;for(const a of this.actors)if(!a.downUntil&&inStrike(player,a,s.direction,attackStats(s.weapon,s.kind),this.mover))this.hit(a,attackStats(s.weapon,s.kind).damage)}
