...
export function providerInfo(env: NodeJS.ProcessEnv = process.env): ProviderInfo {
  ...
  if (selection === 'openai') {
    ...
    const available = Boolean(env.OPENAI_API_KEY) && !error;
  ...
  if (selection === 'claude-cli') {
    ...
    return { provider: 'claude-cli', model, available: found && !error, label: !found ? 'Claude CLI not found' : error ?? `Claude CLI ${model} · local login; requests verify access` };
  }
...
function timeoutMs(): number {
  const configured = Number(process.env.ASTRA_MODEL_TIMEOUT_MS || 120_000);
  return Number.isFinite(configured) ? Math.min(180_000, Math.max(1_000, configured)) : 120_000;
}

async function claudeDecision(prompt: string, model: string): Promise<unknown> {
  const bin = executable(process.env);
  ...
  return new Promise((resolve, reject) => {
    ...
    ], { shell: false, cwd: tmpdir(), stdio: ['pipe', 'pipe', 'pipe'], env: process.env });
    ...
    const timer = setTimeout(() => finish(new Error('Claude CLI timed out. Your input has not been converted to an action.')), timeoutMs());
...
async function runDecision(view: ActorView, request: RequestKind): Promise<ModelResult> {
  ...
  try {
    let raw: unknown;
    if (info.provider === 'claude-cli') raw = await claudeDecision(buildPrompt(view, request), info.model);
...
export async function interpret(view: ActorView, input: string): Promise<ModelResult> {
  ...
  return requestDecision(view, { kind: 'player', input });
}
export async function decideNpc(view: ActorView): Promise<ModelResult> {
  if (view.actor.role !== 'npc') throw new Error('NPC decisions require an NPC actor view.');
  return requestDecision(view, { kind: 'npc' });
}
