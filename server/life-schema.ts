...
const resident = z.object({ id, name: z.string().max(100), ...point, role: z.enum(['player', 'npc']), color: z.string(), skin: z.string(), hair: z.string(), traits: z.array(z.string()).max(10), aspiration: z.string(), mood: z.string(), hurt: need.default(0), fear: z.record(z.string(), z.number().finite().min(0).max(100)).default({}), harmDone: z.number().int().nonnegative().max(9999).default(0), kindDone: z.number().int().nonnegative().max(9999).default(0), needs: z.object({ hunger: need, energy: need, social: need, fun: need }), relationships: z.record(z.string(), z.number().finite().min(-100).max(100)), memories: z.array(event).max(32), activity: activity.nullable(), queue: z.array(activity).max(5), facing: z.number().finite(), speech: z.string().nullable(), speechUntil: z.number().finite() });
...
export const snapshotSchema = z.object({ format: z.literal(1), state: z.object({ id, version: z.number().int().nonnegative(), elapsed: z.number().finite().nonnegative(), day: z.number().int().positive(), hour: z.number().int().min(0).max(23), minute: z.number().int().min(0).max(59), speed: z.union([z.literal(0), z.literal(1), z.literal(3)]), theme: z.enum(['loft', 'lantern', 'comic']), title: z.string(), width: z.literal(12), depth: z.literal(10), residents: z.array(resident).length(3), objects: z.array(object).length(8), events: z.array(event).max(80), arrestedUntil: z.number().finite().nonnegative().default(0), arrests: z.number().int().nonnegative().max(9999).default(0), provider: z.object({ name: z.string(), model: z.string(), available: z.boolean(), busy: z.boolean(), error: z.string().nullable(), lastLatencyMs: z.number().nonnegative().nullable(), calls: z.number().int().nonnegative() }) }), receipts: z.array(z.tuple([id, z.object({ worldId: id, fingerprint: z.string(), message: z.string() })])).max(256) }).superRefine((value, ctx) => {
