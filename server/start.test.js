...
test('production serves frontend and generation API in the same process',async t=>{
 ...
 const server=productionServer({root,env:{OPENAI_API_KEY:'test',OPENAI_MODEL:'test'},fetcher:async()=>Response.json({status:'completed',output:[{content:[{type:'output_text',text:'{"name":"Sable"}'}]}]})});
